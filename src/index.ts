...
const server = new McpServer(
	...
	},
	{
		instructions: [
			...
			`Some features, such as preview sites and WordPress.com sync, require WordPress.com authentication. Do NOT attempt to run the login command yourself because the login flow is interactive. Instead, ask the user to run "${ STUDIO_AUTH_LOGIN_COMMAND }" in their own terminal, then retry the operation.`,
			...
			'For designed sites, use wpdev_site_inspect after meaningful visual changes and before the final response. Inspect desktop and mobile when practical. Use the structured report to fix layout overflow, missing content, broken images, console errors, failed requests, weak headings/navigation, and mobile issues. Request screenshots only when visual reasoning is needed, because not all AI clients can use image content.',
